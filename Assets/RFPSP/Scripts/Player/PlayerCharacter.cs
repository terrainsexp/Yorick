...
public class PlayerCharacter : MonoBehaviour {
    ...
    public bool TestDifferentPlayer;
    public int PlayerNo;
	void Start () {
	...
	void Update () {
        ...
        if (TestDifferentPlayer)
        {
            PlayerMesh.material = PlayerMaterials[PlayerNo];
            foreach(GameObject PlayersHeads in PlayerHeads)
            {
                PlayersHeads.SetActive(false);
            }
            PlayerHeads[PlayerNo].SetActive(true);
        }
        else
        {
            ...
            else if (PlayerPrefs.GetInt("Player3") == 1)
            {
                PlayerMesh.material = PlayerMaterials[2];
                foreach (GameObject PlayersHeads in PlayerHeads)
                {
                    PlayersHeads.SetActive(false);
                }
                PlayerHeads[2].SetActive(true);
            }

            else if (PlayerPrefs.GetInt("Player4") == 1)
            {
                PlayerMesh.material = PlayerMaterials[3];
                foreach (GameObject PlayersHeads in PlayerHeads)
                {
                    PlayersHeads.SetActive(false);
                }
                PlayerHeads[3].SetActive(true);
            }

            else if (PlayerPrefs.GetInt("Player5") == 1)
            {
                PlayerMesh.material = PlayerMaterials[4];
                foreach (GameObject PlayersHeads in PlayerHeads)
                {
                    PlayersHeads.SetActive(false);
                }
                PlayerHeads[4].SetActive(true);
            }
        }
