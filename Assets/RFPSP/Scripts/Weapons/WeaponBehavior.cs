...
public class WeaponBehavior : MonoBehaviour {
	...
	void Update(){
        ...
        if (istrueee)
        {
            ...
            RaycastHit hit;
            if (Physics.Raycast(mainCamTransform.position, mainCamTransform.forward, out hit, range))
            {
				...
				if(fireOnRelease && ammo > 0 
					...
					if(!fireOnReleaseState && releaseTimer + releaseTime < Time.time){
						...
						if(PlayerWeaponsComponent.offhandThrowActive && weaponNumber == PlayerWeaponsComponent.grenadeWeapon){
                            int newGrenadevalue;
                            newGrenadevalue = PlayerPrefs.GetInt("grenade");
                            newGrenadevalue -= 1;
                            PlayerPrefs.SetInt("grenade", newGrenadevalue);
    ...
    public void AutoFire(bool value)
    {
        istrueee = value;
    }
