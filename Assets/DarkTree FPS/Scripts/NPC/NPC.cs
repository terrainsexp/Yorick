...
namespace DarkTreeFPS
{
    ...
    public class NPC : MonoBehaviour
    {
       ...
       public int RandomPlayer =0;
        ...
        #endregion
        public bool isTdmPlayer=false;
        public bool isTdmEnemy =false;
        public bool isTdmPlayerEnemy = false;
        ...
        private void Start()
        {
           ...
           // RandomPlayer = Random.Range(0, TotalPlayers.Length);
            player = TotalPlayers[RandomPlayer];
        ...
        public void ApplyHit(RaycastHit hit)
        {
            ...
            if (hit.collider.tag == "Player")
            {
               ...
               // CharacterDamage.instance.ApplyDamage(200, Vector3.zero, transform.position, transform, false, false);
                if (hit.transform.GetComponent<FPSPlayer>())
                {
                    hit.transform.GetComponent<FPSPlayer>().hitPoints -= 5;
                }
            }

            if (hit.collider.tag == "Friend")
            {
                if (hit.transform.GetComponent<DarkTreeFPS.NPC>())
                {
                    hit.transform.GetComponent<DarkTreeFPS.NPC>().GetHit(damage);
                }
            }
        ...
        public void Death()
        {
            ...
            if (isTdmEnemy==true)
            {
                ...
                LevelManager.instance.BlueNumPoints += 1;
                EnemySpawnsManager.instance.SpawnTzZOmbie();
                EnemySpawnsManager.instance.tzEnemyCount = 1;
                this.GetComponent<bl_MiniMapItem>().DestroyItem(true);
            }
            else if (isTdmPlayerEnemy)
            {
                Debug.Log("Generating Enemies");
                ScoreScript.instance.NewRandomObject();
                LevelManager.instance.BlueNumPoints += 1;
                FriendEnemySpawnsManager.instance.SpawnTzZOmbie();
                FriendEnemySpawnsManager.instance.tzEnemyCount = 1;
                this.GetComponent<bl_MiniMapItem>().DestroyItem(true);
            }
             else if (isTdmPlayer == true)
              {
                Debug.Log("Generating Friends");
                LevelManager.instance.RedNumPoints += 1;
                FriendsSpawnsManager.instance.tzEnemyCount =1;
                FriendsSpawnsManager.instance.SpawnZombies();
              }
            else
            {
                ScoreScript.instance.NewRandomObject();
                Statistics.instance.killingEnemies();
